...
public:
	...
	int printTour(vector<petrolPump> &list, int startIdx) {
		int next = startIdx;
		do {
			cout << "[" << list[next].petrolAmount << ","
					<< list[next].nextDistance << "] ";
	...
	int FindTour(vector<petrolPump> &list) {
		...
		int canTravelDistance = list[start].petrolAmount
				- list[start].nextDistance;

		while (start != end || canTravelDistance < 0) {

			while (start != end && canTravelDistance < 0) {
				//queue remove
				canTravelDistance = canTravelDistance
						- (list[start].petrolAmount - list[start].nextDistance);
			...
			// queue add
			canTravelDistance = canTravelDistance
					+ (list[end].petrolAmount - list[end].nextDistance);
			end = (end + 1) % list.size();
		}
