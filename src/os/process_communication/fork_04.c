...
/*
 * The system call wait() is easy. This function blocks the calling
 * process until one of its child processes exits or a signal is
 * received. For our purpose, we shall ignore signals. wait() takes
 * the address of an integer variable and returns the process ID of
 * the completed process. Some flags that indicate the completion
 * status of the child process are passed back with the integer pointer.
 *  One of the main purposes of wait() is to wait for completion of
 *  child processes.
...
2->If there is no child process running when the call to wait() is
 made, then this wait() has no effect at all. That is, it is as if
 no wait() is there.
